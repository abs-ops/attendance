...
&emsp;&emsp;考勤系统目前提供两大块内容：1、实现企业考勤管理。2、实现企业LED公告投放。
...
&emsp;&emsp;可以实现对企业各部门管辖的LED设备各时间段投放的公告内容实施管理。
...
&emsp;&emsp;归属企业表示当前员工归属于哪个企业，对应于企业信息表的企业编号。

### 4.1.2.4、部门-员工表

&emsp;&emsp;部门-员工表用于存储部门与员工对应信息，其中部门编号、员工编号组成联合主键。

&emsp;&emsp;部门编号对应于部门信息表的部门编号。

&emsp;&emsp;员工编号对应于员工信息表的员工编号。
...
&emsp;&emsp;中控设备信息表用于存储中控设备信息。

&emsp;&emsp;归属企业表示当前设备工归属于哪个企业，对应于企业信息表的企业编号。

### 4.1.3.2、考勤机设备信息表

&emsp;&emsp;考勤机设备信息表用于存储考勤机设备信息。

&emsp;&emsp;归属企业表示当前设备工归属于哪个企业，对应于企业信息表的企业编号。

&emsp;&emsp;归属部门表示当前设备工归属于哪个部门，对应于部门信息表的部门编号。

&emsp;&emsp;中控设备表示当前设备工连接到哪个中控设备，对应于中控设备信息表的中控编号。


### 4.1.3.2、LED设备信息表

&emsp;&emsp;LED设备信息表用于存储LED设备信息。
...
&emsp;&emsp;打卡信息表用于存储原始打卡信息。

&emsp;&emsp;打卡类型表示打卡方式，目前主要分为人脸识别与指纹打卡。

### 4.1.4.2、考勤信息表

&emsp;&emsp;考勤信息表用于存储经考勤规则清洗层清洗后的有效打卡信息。

&emsp;&emsp;企业编号对应于企业信息表的企业编号，用于在后续数据量大的时候做数据分片用。

&emsp;&emsp;部门编号对应于部门信息表的部门编号。

&emsp;&emsp;中工编号对应于员工信息表的员工编号。

&emsp;&emsp;打卡类型表示打卡方式，目前主要分为人脸识别与指纹打卡。

&emsp;&emsp;是否打卡表示有没有打卡，其中true-已打卡、faluse-未打卡。

&emsp;&emsp;打卡结果用于存储经过考勤规则清洗后的打卡结果，主要有迟到、早退、旷工、请假、外出、正常等。

### 4.1.4.3、考勤规则信息表

&emsp;&emsp;考勤规则信息表用于存储考勤规则信息。

&emsp;&emsp;考勤规则内容用于表示考勤的时间段或者时间点。

&emsp;&emsp;考勤规则备注用于说明考勤规则。

### 4.1.4.4、企业-考勤规则表

&emsp;&emsp;企业-考勤规则表用于企业的考勤规则信息，其中企业编号、考勤规则编号组成联合主键。

&emsp;&emsp;企业编号对应于企业信息表的企业编号。

&emsp;&emsp;考勤规则编号对应于考勤规则信息表的考勤规则编号。

### 4.1.4.5、部门-考勤规则表

&emsp;&emsp;部门-考勤规则表用于企业的考勤规则信息，其中部门编号、考勤规则编号组成联合主键。

&emsp;&emsp;部门编号对应于部门信息表的企业编号。
...
&emsp;&emsp;请假信息表用于存储员工的请假信息。

&emsp;&emsp;部门编号表示员工所在部门，对应于部门信息表的企业编号。

&emsp;&emsp;企业编号表示员工所在企业，对应于企业信息表的企业编号。

&emsp;&emsp;请假类型主要有病假、事假等。

&emsp;&emsp;请假是否有效表示请假是否生效。


### 4.1.4.6、外出登息表

&emsp;&emsp;外出信息表用于存储员工的外出信息。

&emsp;&emsp;部门编号表示员工所在部门，对应于部门信息表的企业编号。

&emsp;&emsp;企业编号表示员工所在企业，对应于企业信息表的企业编号。

&emsp;&emsp;外出类型主要有外勤、出差等。

&emsp;&emsp;外出是否有效表示外出是否生效。
...
&emsp;&emsp;公告信息表用于存储公告信息。

### 4.1.5.1、企业-公告表

&emsp;&emsp;企业-公告表用于存储企业公告信息。

&emsp;&emsp;企业编号表示公告针对哪个企业，对应于企业信息表的企业编号。

&emsp;&emsp;公告编号对应于公告信息表的公告编号。
